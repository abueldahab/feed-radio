...
class MediaData {
    ...
    function get() {
        ...
        if ( $this->mysqli->multi_query($items_query) && $result = $this->mysqli->store_result() ) {
            ...
            $result->free();
            echo "var itemsJson = " . json_encode(unserialize($items));
        } else {
            echo "// failed to retrieve stored items";
        }
        $this->mysqli->close();
    }
    ...
    function updateNecessary() {
        # do update if previous update happened > 1 minute ago
        $timestamp_query = "SELECT TIMEDIFF('00:01:00', TIMEDIFF(NOW(), timestamp)) < 0 FROM data_cache WHERE data_name = 'items'";
    ...
    function storeYoutubeAPIData() {
        ...
        foreach ( $this->youtube_entries as $batch_entries ) {
            ...
            foreach ( $batch_doc->getElementsByTagNameNS("http://www.w3.org/2005/Atom", "entry") as $entry ) {
                ...
                if ( $title == "Error" || preg_match('/interview|episode|nardwuar|vlog|talks/i', $title) ) 
                    continue;
    ...
    function storeSoundcloudAPIData($id, $track) {
        ...
        if ( !isset($response->stream_url) || !isset($response->artwork_url) ) {
            $remove[] = $id;
            return;
        }
